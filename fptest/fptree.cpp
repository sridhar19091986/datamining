...
int FPtree::processItems(Transaction *t, int times)//times计算支持数
{
  ...
  for(int depth=0; depth < t->length; depth++) 
  {
    ...
    if(head == header.end())
	{
      head = header.insert(Item(t->t[depth], 0)).first;//构造Item对象，仅赋值id
      added++;
    }
...
int FPtree::processTransaction(Transaction *t, int times)
{
  ...
  for(int depth=0; depth < t->length; depth++) 
  {
    ...
    if(head != header.end()) 
	{
      it = items->find(Item(t->t[depth], 0));//root集合中有无项

      if(it == items->end()) 
	  {
		it = items->insert(Item(t->t[depth], current)).first;//fp树无该节点，则创建该节点 
		it->setNext(head->getNext());
		head->setNext(it->getItem());//数据结构中的链表添加节点
		nodes++;
		added++;
		if(singlepath && (items->size()>1)) singlepath=false; 	//判断节点数大于1或单路径为真	
      }

      it->Increment(times);
      current = it->getItem();
      items = it->makeChildren();//替换root集合，即遍历字节点
    }
  }
...
int FPtree::grow(int *current, int depth)
{
  ...
  else 
  {
    for(set<Item>::iterator it=header.begin(); it != header.end(); it++) 
	{
      ...
      for(i = it->getNext(); i != NULL; i = i->nodelink) 
	  {
		int l=0;
		for(Item_ *p=i->parent; p; p = p->parent)
		{
			tmp[l++] = p->id;
		}
...
void FPtree::ReOrder()
{
  set<Item>::iterator itI;//集合中项的类型定义的对象，可以操作集合读写
  ...
  for(itE=list.begin(); itE!=list.end(); itE++)
  {
	...
	{
      remap[i] = itE->id;//频繁1-项集的序号
      relist->insert(Element(itE->id,i));//？
      Item a(i,0);//构造函数中参数i代表id，0代表父结点
      itI = header.insert(a).first;//header保存值由候选1-项集变为频繁1-项集
      itI->Increment(itE->support);//itE的支持数输入itI
      i++;
    }
  }
}
