...
{                               /* --- create an item set reporter */
  ...
  char       buf[4*TA_MAXLEN+4];/* buffer for formatting */
  ...
  *rep->pos    = NULL;          /* organize the pointer arrays */
  ...
  rep->pxpp  = (int*)malloc((n+n+n+n+2+b) *sizeof(int));
  if (!rep->pxpp) { isr_delete(rep, 0); return NULL; }
  rep->pexs  = rep->pxpp  +n+1; /* allocate memory for the arrays */
  rep->items = rep->pexs  += n; /* and organize it (split it) */
  rep->supps = rep->items +n;   /* set the integer support array */
  rep->supps[0] = base->wgt;    /* and store the empty set support */
  memset(rep->pxpp, 0, n *sizeof(int));  /* clear item usage flags */
  ...
  for (i = sum = 0; i < n; i++) {
    ...
    sum += k = (mode & ISR_SCAN) ? scn_format(buf, name, 0) : len;
  ...
  rep->buf    = rep->supps+n+1; /* set the second item set buffer */
...
{                               /* --- delete an item set reporter */
  ...
  for (i = 0; rep->inames[i]; i++)   /* traverse the item names */
...
int isr_addx (ISREPORT *rep, int item, int supp, double wgt)
{                               /* --- add an item (support & weight) */
  assert(rep && rep->wgts       /* check the function arguments */
  &&    (item >= 0) && (item < rep->supps -rep->items));
  if (isr_uses(rep, item))      /* if the item is already in use, */
    return -2;                  /* abort the function */
  #ifdef ISR_CLOMAX             /* if closed/maximal filtering */
  if (rep->clomax) {            /* if a closed/maximal filter exists */
...
{                               /* --- output an item set */
  ...
  isr_sinfo(rep, rep->supps[rep->cnt],
            (rep->wgts) ? rep->wgts[rep->cnt] : 0, eval);
...
void isr_direct (ISREPORT *rep, const int *items, int n, int supp,
                 double wgt, double eval)
{                               /* --- report an item set */
...
void isr_directx (ISREPORT *rep, const int *items, int n, double *iwgts,
                  int supp, double wgt, double eval)
{                               /* --- report an item set */
  int c;                        /* buffer for the item counter */
...
int isr_wgtout (ISREPORT *rep, int supp, double wgt)
...
int isr_sinfo (ISREPORT *rep, int supp, double wgt, double eval)
