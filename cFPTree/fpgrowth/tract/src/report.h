...
#define ISR_NOEXP      0x04     /* do not expand perfect extensions */
#define ISR_WEIGHTS    0x08     /* allow for item set weights */
#define ISR_LOGS       0x10     /* compute sums of logarithms */
#define ISR_SCAN       0x20     /* report in scanable form */
...
extern int        isr_addx     (ISREPORT *rep, int item, int supp,
                                double wgt);
...
extern CCHAR*     isr_iname    (ISREPORT *rep, int item);
...
extern int        isr_wgtout   (ISREPORT *rep, int supp, double wgt);
extern int        isr_sinfo    (ISREPORT *rep, int supp, double wgt,
                                double eval);
...
#define isr_iname(r,i)    ((r)->names[i])
