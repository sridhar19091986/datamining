...
static const char *_msgs[] = {  /* error messages */
  /* E_NONE      0 */  "no error",
...
int scn_fmtlen (const char *s, int *len)
...
int scn_format (char *dst, const char *src, int quotes)
...
{                               /* --- get the next character */
  ...
  if (scan->next >= scan->end){ /* if no more characters available */
    int n = fread(scan->buf, sizeof(char), SCN_BUFSIZE, scan->file);
...
{                               /* --- push back a character */
  ...
  return *--scan->next = c;     /* return the pushed back character */
}  /* scn_ungetc() */

/*--------------------------------------------------------------------*/

static int _swap (SCANNER *scan)
...
{                               /* --- check for end of file */
  ...
  scan->token = E_GARBAGE;      /* otherwise set the error status */
  if (reperr) scn_error(scan, scan->token);
...
{                               /* --- report a scan/parse error */
  ...
  if (scan->errfile && code) {  /* if an error file is set */
    ...
    if      (code >  0)         /* if a message is given, print it */
      ...
      msg = _msgs[i = -code];   /* get the error message (format) */
      fputc('\n',scan->errfile);/* start a new output line */
      if (*msg == '#') { msg++; /* print a message header if needed */
        fprintf(scan->errfile, "%s:%d: ", scan->name, scan->line); }
      ...
      if (!msg) msg = _msgs[(int)(sizeof(_msgs)/sizeof(char*))-1];
      fputc('\n',scan->errfile);/* start a new output line */
      if (*msg == '#') { msg++; /* print a message header if needed */
        fprintf(scan->errfile, "%s:%d: ", scan->name, scan->line); }
