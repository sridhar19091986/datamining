...
{                               /* --- create a memory system */
  ...
  ms->mbsz = cnt *ms->size +2;  /* 2: succ. and pred. pointer */
  ...
  ms->cap  = ms->top  = ms->err = 0; ms->stack = NULL;
...
int ms_push (MEMSYS *ms)
{                               /* --- store the current state */
  int     n;                    /* new stack size */
  ...
  s->used = ms->used;
  return ++ms->top;             /* return the new stack size */
}  /* ms_push() */

/*--------------------------------------------------------------------*/

int ms_pop (MEMSYS *ms)
