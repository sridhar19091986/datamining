/*----------------------------------------------------------------------
  ...
  History : 1996.02.14 file created as apriori.c
	    ...
	    Changes made by Li Ma
	    2008.06.05 all item comparison functions are modified to
	    compare based on frequency among cases
----------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{                               /** --- compare item frequencies **/
  ...
  #ifdef ARCH64
  if (((const ITEM*)p1)->frq_among_cases < (long)data)
    return (((const ITEM*)p2)->frq_among_cases < (long)data) ? 0 : 1;
  if (((const ITEM*)p2)->frq_among_cases < (long)data) return -1;
  #else
  if (((const ITEM*)p1)->frq_among_cases < (int)data)
    return (((const ITEM*)p2)->frq_among_cases < (int)data) ? 0 : 1;
  if (((const ITEM*)p2)->frq_among_cases < (int)data) return -1;
  #endif
...
{                               /** --- compare item frequencies **/
  ...
  if (((const ITEM*)p2)->frq_among_cases < (int)data) return -1;
  #endif
  if (((const ITEM*)p1)->frq_among_cases > ((const ITEM*)p2)->frq_among_cases) return  1;
  if (((const ITEM*)p1)->frq_among_cases < ((const ITEM*)p2)->frq_among_cases) return -1;
...
{                               /* --- create an item set */
  ...
  iset->ctac = 0; /** initialize case counter **/
...
int is_read (ITEMSET *iset, FILE *file, int case_label)
{                               /** --- read a transaction **/
...
{                  /** --- recode items w.r.t. frequency among cases **/
...
int tas_add (TASET *taset, const int *items, int n, int perm_label) /** permutaion label added **/
{                               /* --- add a transaction */
  ...
  taset->cnt_cases += ta->perm_label = perm_label;  /** copy the permutation label for this transection **/
