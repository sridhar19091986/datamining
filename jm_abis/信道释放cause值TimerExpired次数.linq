...
var rl=from p in Recovered_Sheet1s
       ...
       group p by p.BSC==null into ttt
       select new 
	   {
	   
	      ttt.Key,
		  信道释放cause值TimerExpired次数=all,
	      SDCCH掉话=ttt.Sum(e=>Convert.ToInt32(e.SDCCH掉话)),
		  TCH掉话=ttt.Sum(e=>Convert.ToInt32(e.TCH掉话)),
		  未接通=ttt.Sum(e=>Convert.ToInt32(e.未接通)),
		  	      SDCCH掉话占比=1.0*ttt.Sum(e=>Convert.ToInt32(e.SDCCH掉话))/all,
		  TCH掉话占比=1.0*ttt.Sum(e=>Convert.ToInt32(e.TCH掉话))/all,
		  未接通占比=1.0*ttt.Sum(e=>Convert.ToInt32(e.未接通))/all,
