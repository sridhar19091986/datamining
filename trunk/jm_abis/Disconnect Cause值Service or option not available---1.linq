...
void Main()
{
	...
	double total = tables.Where(e => e.Message_Ty == 37).Count();

	var cause = from p in tables
				where p.Message_Ty== 37
				group p by new {p.Message_Ty, p.Cause_type, p.Cause_Valu, p.Protocol} into ttt

				select new

	{
		Message_Type_Code = ttt.Key.Message_Ty, //{ 0x25,	"Disconnect" },
		Message_Type = "Disconnect", //{ 0x25,	"Disconnect" },
		Cause_type = ttt.Key.Cause_type,
		Cause_Value = (int)ttt.Key.Cause_Valu,
