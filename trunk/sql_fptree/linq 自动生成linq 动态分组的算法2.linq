...
string linqdump="";
Type type = typeof(Gn_Connect2x);
foreach (var fields in type.GetFields())
{
	string field = "p."+fields.Name;
	linqdump=@"
	var m"+fields.Name+@" = from p in Gn_Connect2xes.ToList()
			   group p by "+field+@" into ttt
			   select new
	{
		"+fields.Name+@" = ttt.Key,
		"+fields.Name+@"_Count = ttt.Count(),
		"+fields.Name+@"_Repeat_Count = ttt.Where(e => e.SYN_Repeat > 0).Count(),
		"+fields.Name+@"_Repeat_Rate = 1.0 * ttt.Where(e => e.SYN_Repeat > 0).Count() / ttt.Count(),
	};
